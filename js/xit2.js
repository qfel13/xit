var xit = (function(document) {
	...
	var cEnergy,
		...
		debug = {keydownHandler: false, parseLevel: true},
	...
	function initDom(size) {
		...
		renderBoard.style.width = 32 * size + "px";
	...
	function parseLevel(s) {
		var i, j, l, ll, k, line, lines, className, version;
			...
			//lines.pop(); // remove last empty entry FIXME: check if that line is empty
			version = lines.shift();
			...
			for (i = 0; i < k; i += 1) {
				line = lines[i];
				if(debug.parseLevel) { console.log("line", line); }
				ll = line.length;
				if ((ll / 2) > levelSize) {
	...
	function keydownHandler(event) {
		var k = event.keyCode, p = false, ctrl = event.ctrlKey, shift = event.shiftKey, level, body, cl;
